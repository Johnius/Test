...
class RecordProcessor(processor.RecordProcessorBase):
    ...
    * shutdown will be called if this MultiLangDaemon instance loses the lease to this shard, or the shard ends due
    ...
    def __init__(self):
        ...
        self._largest_seq = (0, 0)
        self._largest_sub_seq = 0
        ...
        for n in range(0, self._CHECKPOINT_RETRIES):
            try:
                ...
                if 'ShutdownException' == e.value:
                    ...
                    #
                    print('Encountered shutdown exception, '
                          'skipping checkpoint')
                    return
                    ...
                    if self._CHECKPOINT_RETRIES - 1 == n:
                        sys.stderr.write('Failed to checkpoint after {n} '
                                         'attempts, giving up.\n'.format(n=n))
                        return
                    else:
                        print('Was throttled while checkpointing, will '
                              'attempt again in {s} seconds'
    ...
    def process_record(self, data, partition_key, sequence_number, sub_sequence_number):
        ...
        ####################################
        if sub_sequence_number < 3:
            raise Exception("Test exception")
        print('Working on %s for seqeunce number '
              '%s (%s) of partition %s' % (data, sequence_number, sub_sequence_number, partition_key))
        time.sleep(1)
        return
    ...
    def process_records(self, process_records_input):
        ...
        """
        try:
            print('Starting process records %s records.' % len(process_records_input.records))
            ...
            #
            if time.time() - (
                self._last_checkpoint_time > self._CHECKPOINT_FREQ_SECONDS
            ):
